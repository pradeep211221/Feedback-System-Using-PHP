    ...
    if (!empty($email) && !empty($password) && !is_numeric($email))
    {
      ...
      $result=mysqli_query($con,$query);
        if($result && mysqli_num_rows($result)>0)
        {
            ...
            if($userdata["Password_"] == $password)
            {
    ...
    <div class="container">
        ...
        <form action="studlogin.php" method="post">
            ...
            <div class="show-password">
                <!-- Password display here when Show Password checkbox is checked -->
            </div>
            <div class="form-group">
                <input type="submit" value="Login" name="submit">
            </div>
        </form>
        <p>Dont have a account?<a href="registration.php">Register</a></p>
    </div>
    <script>
        ...
        showPasswordCheckbox.addEventListener("change", function () {
            passwordInput.type = this.checked ? "email" : "password";
        });
    </script>
</body>
</html